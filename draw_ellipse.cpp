...
void draw_ellipse_outline(ref_image dest, int origin_x, int origin_y, int width, int height, const colour& c)
{
  ...
  // do the horizontal diameter
//  for (int x = -width; x <= width; x++)
//    draw_point(dest, origin_x + x, origin_y, c);

  draw_point(dest, origin_x - width, origin_y, c);
  draw_point(dest, origin_x + width, origin_y, c);
  ...
  for (int y = 1; y <= height; y++)
  {
    ...
    for (int x = -x0; x < -x1; x++)
    {
        draw_point(dest, origin_x + x, origin_y - y, c);
        draw_point(dest, origin_x + x, origin_y + y, c);
    }
    for (int x = x1; x < x0; x++)
    {
...
/*
    draw_point(dest, origin_x - x0, origin_y - y, c);
    draw_point(dest, origin_x + x0, origin_y + y, c);
    draw_point(dest, origin_x - x0, origin_y + y, c);
    draw_point(dest, origin_x + x0, origin_y - y, c);
*/
